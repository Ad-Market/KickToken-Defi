...
const FarmCards: React.FC = () => {
  ...
  const kktIndex = farms.findIndex(
    ({ tokenSymbol }) => tokenSymbol === 'KKT',
  )

  const kktPrice =
    kktIndex >= 0 && stakedValue[kktIndex]
      ? stakedValue[kktIndex].tokenPriceInWeth
      : new BigNumber(0)

  const BLOCKS_PER_YEAR = new BigNumber(2336000)
  const KKT_PER_BLOCK = new BigNumber(100)
...
const FarmCard: React.FC<FarmCardProps> = ({ farm }) => {
  ...
  }, [kkt, lpTokenAddress, account, setHarvestable])

  const poolActive = true // startTime * 1000 - Date.now() <= 0

  return (
    <StyledCardWrapper>
      {farm.tokenSymbol === 'KKT' && <StyledCardAccent />}
      <Card>
        <CardContent>
          <StyledContent>
